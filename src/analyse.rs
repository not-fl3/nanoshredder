...
impl<'a> DrawShaderAnalyser<'a> {
    ...
    pub fn analyse_shader(&mut self) -> Result<(), LiveError> {
        ...
        for field in &self.shader_registry.draw_shader_def.fields {
            self.analyse_field_decl(field)?;
        }

        // first analyse decls
        for fn_node_ptr in &self.shader_registry.draw_shader_def.methods {
            let fn_def = self.shader_registry.all_fns.get(fn_node_ptr).unwrap();
        ...
        for pixel_fn in &pixel_fns {
            ...
            if let Some(fn_def) = self.shader_registry.all_fns.get(pixel_fn) {
                if let Some(FnSelfKind::DrawShader) = fn_def.self_kind {
                    // lets iterate all
        ...
        *self
            ...
            .draw_shader_def
            .all_live_refs
            .borrow_mut() = all_live_refs;

        *self.shader_registry.draw_shader_def.all_fns.borrow_mut() = all_fns;
        *self.shader_registry.draw_shader_def.vertex_fns.borrow_mut() = vertex_fns;
        *self.shader_registry.draw_shader_def.pixel_fns.borrow_mut() = pixel_fns;

        *self
            .shader_registry
            .draw_shader_def
            .all_structs
            .borrow_mut() = all_structs;
        *self
            .shader_registry
            .draw_shader_def
            .vertex_structs
            .borrow_mut() = vertex_structs;
        *self
            .shader_registry
            .draw_shader_def
            .pixel_structs
            .borrow_mut() = pixel_structs;
        Ok(())
    }
