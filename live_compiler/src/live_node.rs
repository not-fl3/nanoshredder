...
pub struct LiveNode {
    // 40 bytes. Don't really see ways to compress
...
impl FittedString {
    ...
    pub fn as_str<'a>(&'a self) -> &'a str {
        unsafe {
            std::str::from_utf8_unchecked(std::slice::from_raw_parts(self.buffer, self.length))
        }
    }
}
...
impl LiveValue {
    ...
    pub fn is_enum(&self) -> bool {
        match self {
            Self::BareEnum { .. } | Self::TupleEnum { .. } | Self::NamedEnum { .. } => true,
            _ => false,
        }
    }
