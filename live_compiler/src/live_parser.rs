use {
    crate::{
        ...
        live_node::{
            LiveBinOp, LiveEditInfo, LiveNode, LiveNodeOrigin, LivePropType, LiveTypeInfo,
            LiveUnOp, LiveValue,
        },
        ...
        makepad_live_tokenizer::{Delim, LiveId},
        makepad_math::{Vec2, Vec3, Vec4},
        span::{TextPos, TextSpan},
    },
    std::{iter::Cloned, slice::Iter},
};
...
impl<'a> LiveParser<'a> {
    pub fn new(tokens: &'a [TokenWithSpan], live_type_infos: &'a [LiveTypeInfo]) -> Self {
...
impl<'a> LiveParser<'a> {
    ...
    fn possible_edit_info(
        &mut self,
        ld: &mut LiveOriginal,
    ) -> Result<Option<LiveEditInfo>, LiveError> {
                        ...
                        match self.peek_token() {
                            ...
                            LiveToken::String { index, len } => {
                                ...
                                ld.edit_info.push(LiveNode {
                                    origin: LiveNodeOrigin::from_token_id(self.get_token_id()),
                                    id: prop_id,
                                    value: LiveValue::DocumentString {
    ...
    fn get_token_id(&self) -> LiveTokenId {
        LiveTokenId::new(LiveFileId(0), self.token_index)
    }
    ...
    fn expect_live_value(
        ...
        match self.peek_token() {
            ...
            LiveToken::Ident(base) => {
                // we're gonna parse a class or an enum
                self.skip_token();
                if self.accept_token(LiveToken::Punct(id!(::))) {
                    // enum
        ...
        while self.peek_token() != LiveToken::Eof {
            match self.peek_token() {
                LiveToken::Close(Delim::Brace) => {
                    if root {
                        return Err(self
                            .error(format!("Unexpected token }} in root"), live_error_origin!()));
                    }
                    let token_id = self.get_token_id();
                    self.skip_token();
                    ld.nodes.push(LiveNode {
                ...
                LiveToken::Ident(prop_id) => {
                    ...
                    if let LiveToken::Ident(_) = self.peek_token() {
                        ...
                        if
                        /*prop_id.is_capitalised() &&*/
                        self.accept_token(LiveToken::Open(Delim::Brace)) {
                            let origin = LiveNodeOrigin::from_token_id(token_id)
                                .with_edit_info(edit_info)
    ...
    pub fn expect_prop_type(&mut self) -> Result<LivePropType, LiveError> {
        Ok(if self.accept_token(LiveToken::Punct(id!(:))) {
            ...
            return Err(self.error(
                format!("Unexpected assign_type, expected = or :"),
                live_error_origin!(),
            ));
        })
    }
